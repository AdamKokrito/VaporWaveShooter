...
@export var weapon_decay : float = 0.3
...
@export_range(0.0,0.4,0.001) var bob_up : float = 0.05
...
func calculate_view_offset(delta)-> void:
	...
	if speed > 0.1  and player.is_on_floor():
		...
		_step_timer = 0.0
	var bob_sin : float = sin(_step_timer * 2.0 * PI) * 0.5
	...
	if enable_damage_kick:
		var damage_ratio : float = max(0.0,_damage_timer/damage_time)
		
		angles.x += damage_ratio * _damage_pitch
		offset.y += damage_ratio * _damage_roll
	
	if _weapon_kick_angles:
...
func update_screen_shake(alpha:float , amount :float) ->void :
	...
	var current_shake_amount : float = amount * (1.0 - alpha)
